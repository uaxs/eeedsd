...
**Commands:**\n
> :space_invader: `{prefix}lain` - Montrer mes Réseaux Sociaux.
> :wrench: `{prefix}changeprefix <prefix>` - Changer le Préfix.  
> :x: `{prefix}shutdown` - Arreter le Selfbot.  
> :notepad_spiral: `{prefix}uptime` - Affiche depuis combien de temps le selfbot tourne.
> :closed_lock_with_key: `{prefix}remoteuser <@user>` - Autoriser une personne a executer des commandes de loin.
> :robot: `{prefix}copycat ON|OFF <@user>` - Repond automatiquement un message d'AFK quand quelqu'un te DM. 
> :pushpin: `{prefix}ping` - Affiche le Ping du Selfbot.
> :pushpin: `{prefix}pingweb <url>` - Ping un site et affiche la réponse (e.g., 200 si il est connecté).
> :gear: `{prefix}geoip <ip>` - Lookup les informations d'une IP.
> :microphone: `{prefix}tts <text>` - Convertit le texte et envoie un fichier audio (.wav).
> :hash: `{prefix}qr <text>` - Générez un code QR à partir du texte fourni et envoyez-le sous forme d'image.
> :detective: `{prefix}hidemention <display_part> <hidden_part>` - Masquer les messages à l'intérieur d'autres messages.
> :wrench: `{prefix}edit <message>` - Déplacer la position du logo (modifié).
> :arrows_counterclockwise: `{prefix}reverse <message>` - Inverser les lettres d'un message.
> :notepad_spiral: `{prefix}gentoken` - Générer un token invalide mais correctement structuré.
> :woozy_face: `{prefix}hypesquad <house>` - Changez votre badge HypeSquad.
> :dart: `{prefix}nitro` - Genere un faux lien Nitro.
> :hammer: `{prefix}whremove <webhook_url>` - Supprimer un webhook.
> :broom: `{prefix}purge <amount>` - Supprimer un nombre spécifique de messages.
> :broom: `{prefix}clear` - Effacer les messages d'un salon.
> :broom: `{prefix}cleardm <amount>` - Supprimer tous les DM avec un utilisateur."""
    await ctx.send(help_text)

    help_text = f"""
> :writing_hand: `{prefix}spam <amount> <message>` - Spam un message avec le nombre de fois.
> :tools: `{prefix}quickdelete <message>` - Envoyer un message et le supprimer après 2 secondes.
> :tools: `{prefix}autoreply <ON|OFF>` - Activer ou désactiver le autoreply.
> :zzz: `{prefix}afk <ON/OFF>` - Activer ou désactiver le mode AFK. Envoie un message personnalisé lors de la réception d'un message privé ou d'une mention.
> :busts_in_silhouette: `{prefix}fetchmembers` - Récupérer la liste de tous les membres du serveur.
> :scroll: `{prefix}firstmessage` - Obtenez le lien vers le premier message du salon actuel.
> :mega: `{prefix}dmall <message>` - Envoyer un message à tous les membres du serveur.
> :mega: `{prefix}sendall <message>` - Envoyer un message dans tous les salons du serveur.
> :busts_in_silhouette: `{prefix}guildicon` - Obtenir l'icône du serveur actuel.
> :space_invader: `{prefix}usericon <@user>` - Obtenir la photo de profil d'un utilisateur.
> :star: `{prefix}guildbanner` - Obtenir la bannière du serveur actuel.
> :page_facing_up: `{prefix}tokeninfo <token>` - Récupérer des informations d'un token.
> :pager: `{prefix}guildinfo` - Obtenez des informations sur le serveur actuel.
> :memo: `{prefix}guildrename <new_name>` - Renommez le serveur.
> :video_game: `{prefix}playing <status>` - Définissez le statut d'activité du bot sur « Joue à ».  
> :tv: `{prefix}watching <status>` - Définissez le statut d'activité du bot sur « Regarde »".  
> :x: `{prefix}stopactivity` - Réinitialiser le status.
> :art: `{prefix}ascii <message>` - Convertir un message en ASCII.
> :airplane: `{prefix}airplane` - Envoie une attaque du 11 septembre (attention : à utiliser de manière responsable).
> :fire: `{prefix}dick <@user>` - Afficher la « taille » du zgeg d'un utilisateur.
> :x: `{prefix}minesweeper <width> <height>` - Jouez à un jeu de démineur avec une taille de grille personnalisée.
> :robot: `{prefix}leetpeek <message>` - Parlez comme un hacker en remplaçant les lettres."""
    await ctx.send(help_text)
